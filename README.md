...
CookieSync is a pair of browser extensions built from one codebase: a Brave/Chromium build and a Firefox build. The Brave build uploads an encrypted copy of your cookies to a Supabase table you control. The Firefox build downloads that same encrypted blob, decrypts it locally, and lets you choose which sites' cookies actually get written into Firefox.
...
- **Upload is all-or-nothing, import is selective.** The Brave build encrypts and uploads the browser's entire cookie set in one payload. The Firefox build lets you pick exactly which domains from that payload get written back into the browser. See [PRIVACY.md](./PRIVACY.md) for what that means in practice.
...
CookieSync/
├── manifests/
│   ├── brave.json      # Manifest V3, for the publisher build
│   └── firefox.json    # Manifest V2, for the consumer build
├── src/
...
└── scripts/build.mjs           # Builds dist/brave and dist/firefox
```

`scripts/build.mjs` compiles the same source twice, once per manifest, and bakes a `__BROWSER_TARGET__` constant of `"brave"` or `"firefox"` into each bundle at build time. Which role an installed copy plays (publisher or consumer) is fixed by which bundle you installed, not something you switch at runtime.
...
1. Download the file for your browser:
   - **Brave/Chromium:** `cookie-sync-brave-vX.Y.Z.zip`
   - **Firefox:** `cookie-sync-firefox-vX.Y.Z.xpi` if one was published for that release (signed through Mozilla's API), otherwise `cookie-sync-firefox-vX.Y.Z.zip` (unsigned, temporary-load only)
2. Verify the download, since this handles session cookies:
   ```bash
   sha256sum -c checksums.txt --ignore-missing
   ```
3. **Brave/Chromium:** unzip it, open `brave://extensions`, turn on Developer mode, click **Load unpacked**, and select the unzipped folder.
4. **Firefox, signed `.xpi`:** open `about:addons`, click the gear icon, choose **Install Add-on From File**, select the `.xpi`.
5. **Firefox, unsigned `.zip`:** unzip it, open `about:debugging#/runtime/this-firefox`, click **Load Temporary Add-on**, and select `manifest.json` inside the unzipped folder.

> [!NOTE]
> Temporary add-ons in Firefox disappear every time you restart the browser. Whether a signed `.xpi` is available depends on whether the repository's AMO signing secrets were configured for that release, check the release notes.
...
This produces `dist/brave` and `dist/firefox`. Load either the same way as a downloaded release build, steps 3 to 5 above.
...
The popup's title tells you which role a build plays: **Brave Cookie Publisher** or **Firefox Cookie Consumer**.

**Brave, first-time setup:**
...
3. Paste the Sync ID from Brave.
...
**Daily auto-sync**, off by default. Turning it on makes the extension run one push (Brave) or one pull of previously-imported domains (Firefox) the first time the browser starts each calendar day.
...
**Clearing data:** Brave has a **Delete server data** button that removes (or, if the delete is blocked, blanks) the Supabase row for the current Sync ID. Firefox has a **Clear all local cookies** button, and a small delete icon next to each imported site, for removing cookies from the browser itself.
