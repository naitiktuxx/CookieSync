...
In **Offline Workspace** mode, `.cokz` files use the 250,000-iteration key derivation directly and do not attach or embed `auth_hash` headers, ensuring offline payload files contain strictly ciphertext.
...
**Offline Workspace Session Teardown:** The Offline Workspace enforces strict per-session isolation. Whenever an `offline.html` tab is closed or a new offline workspace tab is opened, the in-memory and local storage copies of the `.cokz` snapshot (`offlineSnapshot`) and imported site selections (`importedDomains`) are immediately wiped. If "Remember Passphrase" is disabled, the session passphrase is also erased. Opening an offline tab always requires loading a fresh `.cokz` file.
...
- The Chromium Publisher build has no way, from the popup, to clear its own Cookie Ledger. The Firefox Receiver build has a "Clear all local cookies" action that resets both the browser's cookies and the ledger; Chromium doesn't expose an equivalent.
